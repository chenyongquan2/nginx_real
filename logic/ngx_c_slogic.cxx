...
//定义成员函数指针
typedef bool (CLogicSocket::*CmdHandleFunc)(  NgxConnectionInfo* pConn,      //连接池中连接的指针
                                        NgxExtraMsgHeaderInfo* pMsgHeader,  //消息头指针
                                        char *pPkgBody,                 //包体指针
                                        unsigned short iBodyLength);    //包体长度

//用来保存 成员函数指针 的这么个数组
static const CmdHandleFunc CmdHandleFuncArr[] = 
{
    //数组前5个元素，保留，以备将来增加一些基本服务器功能
    &CLogicSocket::DoCmdPing,                             //【0】：心跳包的实现
    NULL,                                                   //【1】：下标从0开始
    NULL,                                                   //【2】：下标从0开始
    NULL,                                                   //【3】：下标从0开始
    NULL,                                                   //【4】：下标从0开始
    ...
    //开始处理具体的业务逻辑
    &CLogicSocket::DoCmdRegister,                         //【5】：实现具体的注册功能
    &CLogicSocket::DoCmdLogin,                            //【6】：实现具体的登录功能
    //......其他待扩展，比如实现攻击功能，实现加血功能等等；


};
#define AUTH_TOTAL_COMMANDS sizeof(CmdHandleFuncArr)/sizeof(CmdHandleFunc) //整个命令有多少个，编译时即可知道
...
void CLogicSocket::ExecCmd(char *pMsgBuf)
{          
    NgxExtraMsgHeaderInfo* pMsgHeader = (NgxExtraMsgHeaderInfo*)pMsgBuf;                  //消息头
    NgxPkgHeaderInfo*  pPkgHeader = (NgxPkgHeaderInfo*)(pMsgBuf+m_pkgMsgHeaderSize); //包头
    void  *pPkgBody;                                                              //指向包体的指针
    unsigned short pkglen = ntohs(pPkgHeader->pkgLen);                            //客户端指明的包宽度【包头+包体】

    if(m_pkgHeaderSize == pkglen)
    {
        //没有包体，只有包头
    ...
    else 
	{
		...
		pPkgBody = (void *)(pMsgBuf+m_pkgMsgHeaderSize+m_pkgHeaderSize); //跳过消息头 以及 包头 ，指向包体

        //ngx_log_stderr(0,"CLogicSocket::ExecCmd()中收到包的crc值为%d!",pPkgHeader->crc32);

		//计算crc值判断包的完整性        
		int calccrc = CCRC32::GetInstance()->Get_CRC((unsigned char *)pPkgBody,pkglen-m_pkgHeaderSize); //计算纯包体的crc值
		if(calccrc != pPkgHeader->crc32) //服务器端根据包体计算crc值，和客户端传递过来的包头中的crc32信息比较
		{
            ngx_log_stderr(0,"CLogicSocket::ExecCmd()中CRC错误[服务器:%d/客户端:%d]，丢弃数据!",calccrc,pPkgHeader->crc32);    //正式代码中可以干掉这个信息
			return; //crc错，直接丢弃
		}
        else
        {
            //ngx_log_stderr(0,"CLogicSocket::ExecCmd()中CRC正确[服务器:%d/客户端:%d]，不错!",calccrc,pPkgHeader->crc32);
        }        
	}

    //包crc校验OK才能走到这里    	
    unsigned short imsgCode = ntohs(pPkgHeader->msgCode); //消息代码拿出来
    NgxConnectionInfo* p_Conn = pMsgHeader->pConn;        //消息头中藏着连接池中连接的指针
    ...
    {
        ngx_log_stderr(0,"CLogicSocket::ExecCmd()中imsgCode=%d消息码不对!",imsgCode); //这种有恶意倾向或者错误倾向的包，希望打印出来看看是谁干的
        return; //丢弃不理这种包【恶意包或者错误包】
    }

    //能走到这里的，包没过期，不恶意，那好继续判断是否有相应的处理函数
    //(3)有对应的消息处理函数吗
    if(CmdHandleFuncArr[imsgCode] == NULL) //这种用imsgCode的方式可以使查找要执行的成员函数效率特别高
    {
        ngx_log_stderr(0,"CLogicSocket::ExecCmd()中imsgCode=%d消息码找不到对应的处理函数!",imsgCode); //这种有恶意倾向或者错误倾向的包，希望打印出来看看是谁干的
        return;  //没有相关的处理函数
    }

    //一切正确，可以放心大胆的处理了
    //(4)调用消息码对应的成员函数来处理
    (this->*CmdHandleFuncArr[imsgCode])(p_Conn,pMsgHeader,(char *)pPkgBody,pkglen-m_pkgHeaderSize);
    return;	
}
...
//发送没有包体的数据包给客户端
void CLogicSocket::SendNoBodyPkgToClient(NgxExtraMsgHeaderInfo* pMsgHeader,unsigned short iMsgCode)
{
    CMemory  *p_memory = CMemory::GetInstance();

    char *p_sendbuf = (char *)p_memory->AllocMemory(m_pkgMsgHeaderSize+m_pkgHeaderSize,false);
    char *p_tmpbuf = p_sendbuf;
    
	memcpy(p_tmpbuf,pMsgHeader,m_pkgMsgHeaderSize);
	p_tmpbuf += m_pkgMsgHeaderSize;

    NgxPkgHeaderInfo* pPkgHeader = (NgxPkgHeaderInfo*)p_tmpbuf;	  //指向的是我要发送出去的包的包头	
    pPkgHeader->msgCode = htons(iMsgCode);	
    pPkgHeader->pkgLen = htons(m_pkgHeaderSize); 
	pPkgHeader->crc32 = 0;		
    _PushMsgToSendQueue(p_sendbuf);
    return;
}

//----------------------------------------------------------------------------------------------------------
//处理各种业务逻辑
bool CLogicSocket::DoCmdRegister(NgxConnectionInfo* pConn,NgxExtraMsgHeaderInfo* pMsgHeader,char *pPkgBody,unsigned short iBodyLength)
{
    //ngx_log_stderr(0,"执行了CLogicSocket::DoCmdRegister()!");
    ...
    int iRecvLen = sizeof(RegisterInfo); 
    ...
    //(3)取得了整个发送过来的数据
    RegisterInfo* p_RecvInfo = (RegisterInfo*)pPkgBody; 
    ...
    //(5)给客户端返回数据时，一般也是返回一个结构，这个结构内容具体由客户端/服务器协商，这里我们就以给客户端也返回同样的 RegisterInfo 结构来举例    
    //RegisterInfo* pFromPkgHeader =  (RegisterInfo*)(((char *)pMsgHeader)+m_pkgMsgHeaderSize);	//指向收到的包的包头，其中数据后续可能要用到
	NgxPkgHeaderInfo* pPkgHeader;	
	CMemory  *p_memory = CMemory::GetInstance();
	CCRC32   *p_crc32 = CCRC32::GetInstance();
    int iSendLen = sizeof(RegisterInfo);  
    //a)分配要发送出去的包的内存

    //iSendLen = 65000; //unsigned最大也就是这个值
    char *p_sendbuf = (char *)p_memory->AllocMemory(m_pkgMsgHeaderSize+m_pkgHeaderSize+iSendLen,false);//准备发送的格式，这里是 消息头+包头+包体
    //b)填充消息头
    memcpy(p_sendbuf,pMsgHeader,m_pkgMsgHeaderSize);                   //消息头直接拷贝到这里来
    //c)填充包头
    pPkgHeader = (NgxPkgHeaderInfo*)(p_sendbuf+m_pkgMsgHeaderSize);    //指向包头
    pPkgHeader->msgCode = _CMD_REGISTER;	                        //消息代码，可以统一在ngx_logiccomm.h中定义
    pPkgHeader->msgCode = htons(pPkgHeader->msgCode);	            //htons主机序转网络序 
    pPkgHeader->pkgLen  = htons(m_pkgHeaderSize + iSendLen);        //整个包的尺寸【包头+包体尺寸】 
    //d)填充包体
    RegisterInfo* p_sendInfo = (RegisterInfo*)(p_sendbuf+m_pkgMsgHeaderSize+m_pkgHeaderSize);	//跳过消息头，跳过包头，就是包体了
    ...
    */
    //ngx_log_stderr(0,"执行了CLogicSocket::DoCmdRegister()并返回结果!");
    return true;
}
bool CLogicSocket::DoCmdLogin(NgxConnectionInfo* pConn,NgxExtraMsgHeaderInfo* pMsgHeader,char *pPkgBody,unsigned short iBodyLength)
{    
    if(pPkgBody == NULL)     
        return false;
	    
    int iRecvLen = sizeof(LoginInfo); 
    if(iRecvLen != iBodyLength)   
        return false; 

    CLock lock(&pConn->logicPorcMutex);
        
    LoginInfo* p_RecvInfo = (LoginInfo*)pPkgBody;     
    ...
    int iSendLen = sizeof(LoginInfo);  
    char *p_sendbuf = (char *)p_memory->AllocMemory(m_pkgMsgHeaderSize+m_pkgHeaderSize+iSendLen,false);    
    memcpy(p_sendbuf,pMsgHeader,m_pkgMsgHeaderSize);    
    pPkgHeader = (NgxPkgHeaderInfo*)(p_sendbuf+m_pkgMsgHeaderSize);
    pPkgHeader->msgCode = _CMD_LOGIN;
    pPkgHeader->msgCode = htons(pPkgHeader->msgCode);
    pPkgHeader->pkgLen  = htons(m_pkgHeaderSize + iSendLen);    
    LoginInfo* p_sendInfo = (LoginInfo*)(p_sendbuf+m_pkgMsgHeaderSize+m_pkgHeaderSize);
    pPkgHeader->crc32   = p_crc32->Get_CRC((unsigned char *)p_sendInfo,iSendLen);
    pPkgHeader->crc32   = htonl(pPkgHeader->crc32);		   
...
//接收并处理客户端发送过来的ping包
bool CLogicSocket::DoCmdPing(NgxConnectionInfo* pConn,NgxExtraMsgHeaderInfo* pMsgHeader,char *pPkgBody,unsigned short iBodyLength)
{
    //心跳包要求没有包体；
