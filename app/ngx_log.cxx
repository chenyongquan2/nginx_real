...
{    
    ...
    *p++ = '\n'; //增加个换行符    
    ...
    if(g_ngxLogInfo.fd > STDERR_FILENO) //如果这是个有效的日志文件，本条件肯定成立，此时也才有意义将这个信息写到日志文件
    {
...
void ngx_log_error_core(int level,  int err, const char *fmt, ...)
{
    ...
    p = ngx_slprintf(p, last, "%P: ",g_curPid);                             //支持%P格式，进程id增加进来，得到形如：   2019/01/08 20:50:15 [crit] 2037:
    ...
    *p++ = '\n'; //增加个换行符       
    ...
    while(1) 
    {        
        if (level > g_ngxLogInfo.log_level) 
        {
        ...
        //写日志文件        
        n = write(g_ngxLogInfo.fd,errstr,p - errstr);  //文件写入成功后，如果中途
        if (n == -1) 
        {
            //写失败有问题
            ...
            else
            {
                ...
                if(g_ngxLogInfo.fd != STDERR_FILENO) //当前是定位到文件的，则条件成立
                {
...
void ngx_log_init()
{
    ...
    g_ngxLogInfo.log_level = p_config->GetIntDefault("LogLevel",NGX_LOG_NOTICE);//缺省日志等级为6【注意】 ，如果读失败，就给缺省日志等级
    ...
    //g_ngxLogInfo.fd = open((const char *)plogname,O_WRONLY|O_APPEND|O_CREAT|O_DIRECT,0644);   //绕过内和缓冲区，write()成功则写磁盘必然成功，但效率可能会比较低；
    g_ngxLogInfo.fd = open((const char *)plogname,O_WRONLY|O_APPEND|O_CREAT,0644);  
    if (g_ngxLogInfo.fd == -1)  //如果有错误，则直接定位到 标准错误上去 
    {
        ngx_log_stderr(errno,"[alert] could not open error log file: open() \"%s\" failed", plogname);
        g_ngxLogInfo.fd = STDERR_FILENO; //直接定位到标准错误去了        
    } 
    return;
}
