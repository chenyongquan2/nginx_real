...
//装载配置文件
bool CConfig::Load(const char *pConfFileName) 
{   
    FILE *fp = fopen(pConfFileName,"r");
    if(fp == NULL)
        return false;
    ...
    lblprocstring:
        ...
        if(ptmp != NULL)
        {
            ConfItem* p_confitem = new ConfItem;                    //注意前边类型带LP，后边new这里的类型不带
...
{
	std::vector<ConfItem*>::iterator pos;	
