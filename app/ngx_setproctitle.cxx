...
void NgxMoveEnvironMemPos()
{   
    //这里无需判断penvmen == NULL,有些编译器new会返回NULL，有些会报异常，但不管怎样，如果在重要的地方new失败了，你无法收场，让程序失控崩溃，助你发现问题为好； 
    g_pNewEnvMem = new char[g_envneedmem]; 
    memset(g_pNewEnvMem,0,g_envneedmem);  //内存要清空防止出现问题

    char *ptmp = g_pNewEnvMem;
    //把原来的内存内容搬到新地方来
...
void NgxSetProcessTitle(const char *title)
{
    ...
    size_t esy = g_originArgvsMenSize + g_envneedmem; //argv和environ内存总和
    if( esy <= ititlelen)
    {
    ...
    g_ppOriginArgv[1] = NULL;  

    //(4)把标题弄进来，注意原来的命令行参数都会被覆盖掉，不要再使用这些命令行参数,而且g_os_argv[1]已经被设置为NULL了
    char *ptmp = g_ppOriginArgv[0]; //让ptmp指向g_os_argv所指向的内存
    strcpy(ptmp,title);
    ptmp += ititlelen; //跳过标题
